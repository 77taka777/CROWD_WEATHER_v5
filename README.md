...
## v5.11 の変更点（デバイス表現のリアル化）

ピッチで「実機で動いている」ように見せるため、**ページ → 筐体 → 画面**の
3段階で明度差をつけた（従来は画面内とページ背景がほぼ同色で、枠が単なる
装飾ボーダーに見えていた）。

1. **画面内をページより一段暗く**（`#080D19→#05080F`）。スマホ・タブレット共通。
   画面内のパネル（`#0E1628`）が浮き上がり、可読性も向上する。
2. **ガラス表現**: 画面内側にインナーシャドウ＋上端に1pxハイライト、
   タブレットは発光のにじみをごく弱く付与。
3. **光沢オーバーレイ**: 斜めのハイライトを重ねる（`pointer-events:none` のため
   操作は素通り）。強さは `:root` の `--sheen-a`（既定 .045）の1か所で調整でき、
   **`--sheen-a:0` で無効化**。プロジェクタ投影で白飛びする場合は0推奨。
4. **スマホ筐体をメタル調グラデに**（タブレットと質感を統一）。

## v5.10 の変更点（モバイル表示の修正・PCは変更なし）

1. **横あふれの根本原因を修正**: グリッド子要素に `min-width:0` を付与。
   グリッド項目が最小コンテンツ幅より縮まず、パネル自体が画面幅を超えていた
   （ゾーン状況ボード・給水/避難パネルの見切れの原因）。
2. **ゾーン状況ボードを横スライド対応**: 専用の `.board-wrap`（overflow-x:auto）＋
   モバイルでテーブルに `min-width` を設定し、「この先の予報」と同じ横スクロールに。
   モバイルのみ「→ 横にスクロールできます」のヒントを表示。
3. **会場周辺の給水・避難スポットのモバイル調整**: レーダー地図を最大290pxの正方形に
   （Googleマップ時は高さ215px）、スポット名11px・住所9px・距離11px・番号バッジ18px・
   スイッチ30pxに縮小し、他要素とバランスを調整。出典行も9.5pxに。
   モバイル用CSSがOD行の基本スタイルより前に置かれ効いていなかった問題も修正
   （メディアクエリをスタイルシート末尾へ移動）。
   ※360/390/820/1440px で全ビューの横あふれゼロを確認。

## v5.9 の変更点

1. **ボタン文言**: 現場スタッフ向けの生成ボタンを「現場指示」に変更。
2. **現場指示の1行目に現在のアラートを表示**: アラート生成を `liveAlerts()` に共通化し、
   その時点でアラートボードに出ている内容を1行に要約して文書の冒頭へ自動付与
   （最重要のアラート＋「ほかN件」、アラートなしの場合は平常である旨）。
   AI生成・ルールベース生成のどちらでも付与され、AIには `currentAlerts` として
   アラート内容を渡し、対応指示を最優先で書かせる。
3. **来場者向けの「裏の目的」セクションを更新**: 「いま○○ブースが空いています」等の
   例示テキストを削除し、「ホストページのスポットで、ONのスポットのみを対象にする
   ことで、手動で混雑分散化も可能。」を追記。タグの一番上に
   「安全対策：状況を見て混雑しない場所に誘導」を追加。

## v5.8 の変更点

1. **スポット単位の有効/無効スイッチ**（主催画面）: 給水・避難の各スポット行末に
   トグルスイッチを追加。OFFにしたスポットは主催のマップ（SVG/Googleマップ）から
   消え、**来場者向け（モバイル）のマップ・案内にも表示されなくなる**。
   最寄りへの道順線は有効スポットの中の最寄りに自動で切替。番号はリストと
   マップで一貫（OFF行は打ち消し表示・薄色）。選択状態はブラウザに保存され
   リロード後も維持（localStorage・失敗時は全ONで動作）。
   種別トグル（給水/避難）は従来どおり主催画面の表示フィルタ。

## v5.7 の変更点

1. **主催画面: 給水・避難スポットの表示ON/OFFトグル**。タブ切替をやめ、両方を
   同時表示可能に（リスト・レーダー・Googleマップのマーカーが色分けで統合、
   近い順に通し番号、最寄り①への道順線は表示中スポット全体の最寄りに追従）。
2. **来場者向け（モバイル）に「付近の給水・避難マップ」カードを追加**。
   `GOOGLE_MAPS_API_KEY` 設定時はスマホ画面内にGoogleマップ（給水=緑・避難=橙の
   マーカー、タップで詳細）、未設定時はSVG簡易マップに自動フォールバック。
3. **モバイルレイアウト調整**（600px以下）: ヘッダー・レンズ・KPI・パネル・
   タブレット枠・リスト等を縮小最適化し、横スクロールが発生しないよう修正
   （390px幅で全ビューの横はみ出しゼロを確認）。
4. **現場スタッフ向けの文言変更**: レンズ名を「現場スタッフ向け（タブレット）」に。
   スタッフ配置の注記から「最終配置は運営責任者の確認を前提とする…」の一文を削除。
